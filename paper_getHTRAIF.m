function htrAIF = paper_getHTRAIF(reconraw,t_htr,AMask,cpiVec,aortaDisco)
...
%% DEBUG
% percentRoiData = zeros(size(roiData));
% T = size(roiData,2);
% for tt=1:T
%     cpiInd = cpiVec(tt)+1;
%     percentRoiData(:,tt) = (roiData(:,tt) - preConRoiData(:,cpiInd))./preConRoiData(:,cpiInd);
% end
% 
% figure
% imagesc(percentRoiData(:,cpiVec==0),[0 5])
% 
% selectedVoxels = cell(length(unique(cpiVec)),1);
% for ii = 1:length(unique(cpiVec))
%     cpi = ii-1;
%     [~,peakIndVec] = max(percentRoiData(:,cpiVec==cpi),[],2);
%     [n,xout] = hist(peakIndVec,1:max(peakIndVec));
% %     figure
% %     bar(xout,n)
%     n(1) = 0;
%     [~,selectedIndex] = max(n);
...
% %         selectedIndex = 2;
% %     end
%     selectedVoxels{ii} = find(peakIndVec~=1);
% end
% 
...
%     
%     currentMean = mean(currentRoiData(selectedVoxels{cpiInd}));
%     
%     baseRoiData = preConRoiData(:,cpiInd);
%     baseMean = mean(baseRoiData(selectedVoxels{cpiInd}));
...
% save('trackIndex_good.mat','trackIndex');
% load('trackIndex_good.mat')
...
if (scaleForComparison)
    ...
    comparisonScaling = mean(percentSI_aorta(t_ltr>=timeToStartMatch))/mean(percentChange1Fixed(end-matchLast+1:end));
end


%% Alternative Scaling
[~,discoTailStartInd] = max(percentSI_aorta);
discoTailStart = t_ltr(discoTailStartInd);
% discoTailStart = 43;

discoInterp = interp1(t_ltr,percentSI_aorta,t_htr,'cubic','extrap');
...
percentChangeSmooth = smooth(percentChange1Fixed,0.2,'loess');
...
if (showFigures)
    ...
    figure
    plot(t_ltr,percentSI_aorta,'k-','LineWidth',2)
    hold on
